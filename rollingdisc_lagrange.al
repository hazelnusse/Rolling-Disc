...
% Energy
T = replace(dot(v_co_n>, v_co_n>)/2.0+dot(w_c_n>,dot(I_C_CO>>,w_c_n>))/2.0, &
    wx=dot(wcn_coords>, b1>), &
    wy=dot(wcn_coords>, b2>), &
    wz=dot(wcn_coords>, b3>)) 

V = m*g*dot(p_cn_co>, a3>)

L = rhs(T)-rhs(V)

zero = [dt(d(L, psi')) - d(L, psi), &
        dt(d(L, phi')) - d(L, phi), &
        dt(d(L, theta')) - d(L, theta), &
        dt(d(L, x')) - d(L, x), &
        dt(d(L, y')) - d(L, y)]
